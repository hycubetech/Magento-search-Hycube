...
/**
 * Hycubecompleteplus_Autosuggest_ProductsController
 *
 * NOTICE OF LICENSE
 *
 * This source file is subject to the Open Software License (OSL 3.0)
 ...
 * @link      http://opensource.org/licenses/osl-3.0.php
 */
class Hycubecompleteplus_Autosuggest_ProductsController extends Hycubecompleteplus_Autosuggest_Controller_Abstract
{
    const MISSING_PARAMETER = 'false';
    const PUSH_IN_PROGRESS = 1;
    const PUSH_COMPLETE = 2;
    const POST_MESSAGE_OK = 'ok';
    const URL_EMAIL_UPDATE = 'http://magento.hycubecompleteplus.com/ext_update_email';
    const URL_UUID_UPDATE = 'http://magento.instantsearchplus.com/update_uuid';
    const XML_CONFIG_STORE_EMAIL = 'hycubecompleteplus/config/store_email';
    ...
    {
        Varien_Profiler::start('Hycubecompleteplus_Autosuggest_Products_Send');
        ...
        $catalogModel = Mage::getModel('hycubecompleteplus_autosuggest/catalog');
        ...
        Varien_Profiler::stop('Hycubecompleteplus_Autosuggest_Products_Send');
    }
    ...
    public function versAction()
    {
        ...
        $enabled = Mage::getStoreConfigFlag('hycubecompleteplus/config/enabled', 0);
        ...
        $miniform_change = Mage::getStoreConfig(
            'hycubecompleteplus/config/miniform_change'
        );

        $smart_nav_native = Mage::getStoreConfig(
            'hycubecompleteplus/config/smart_nav_native'
        );

        $external_image = Mage::getStoreConfig(
            'hycubecompleteplus/config/imagefield'
        );
    ...
    public function getNumOfProductsAction()
    {
        $catalogReport = Mage::getModel(
            'hycubecompleteplus_autosuggest/catalogreport'
        );
        $show_out_of_stock = Mage::getStoreConfigFlag(
        ...
        if ($this->validateUuid($key)) {
            ...
            if ($io->fileExists($robotsPath)) {
                ...
                if ($io->isWritable(Mage::getBaseDir())) {
                    ...
                    $command = 'http://magento.hycubecompleteplus.com/install_error';
    ...
    public function pushbulkAction()
    {
        ...
        $pusher = Mage::getModel('hycubecompleteplus_autosuggest/pusher')
            ->load($pushId);
        $sent = $pusher->getSent();
        ...
        $totalPushes = Mage::getModel('hycubecompleteplus_autosuggest/pusher')
            ->getCollection()
            ->getSize();
