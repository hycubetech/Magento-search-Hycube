 ...
 */
class Hycubecompleteplus_Autosuggest_Model_Observer extends Mage_Core_Model_Abstract
{
    const HYCUBECOMPLETEPLUS_WEBHOOK_URI = 'https://acp-magento.appspot.com/ma_webhook';
    const API_UPDATE_URI = 'http://magento.hycubecompleteplus.com/update';
    ...
    {
        $this->imageField = Mage::getStoreConfig('hycubecompleteplus/config/imagefield');
        if (!$this->imageField) {
        ...
        $this->batchesHelper = Mage::helper('hycubecompleteplus_autosuggest/batches');
    }
    ...
    public function catalog_product_save_after_depr($observer)
    {
        ...
        Mage::log($res, null, 'hycubecomplete.log');
    }
    ...
    public function catalog_product_save_after_real($observer)
    {
        ...
        try {
            $updates = Mage::getModel('hycubecompleteplus_autosuggest/batches')->getCollection()
    ...
    public function catalog_product_import_finish_before($observer){
        try {
            ...
            Mage::log($e->getMessage(), null, 'hycubecomplete.log');
        }
    }
    ...
    {
        $notifications = array();
        /** @var Hycubecompleteplus_Autosuggest_Helper_Data $helper */
        $helper = Mage::helper('hycubecompleteplus_autosuggest');
        $command = 'http://magento.hycubecompleteplus.com/ext_info?u='.$this->getConfig()->getUUID();
        ...
        if (!empty($notifications)) {
            Mage::getResourceModel('hycubecompleteplus_autosuggest/notifications')->addNotifications($notifications);
        }
    ...
    {
        /** @var Hycubecompleteplus_Autosuggest_Model_Mysql4_Notifications_Collection $notifications */
        $notifications = Mage::getModel('hycubecompleteplus_autosuggest/notifications')->getCollection();
    ...
    /**
     * @param Hycubecompleteplus_Autosuggest_Model_Notifications $notification
     */
    protected function _sendStatusMail($notification)
    {
        ...
        $storeMail = $helper->getConfigDataByFullPath('hycubecompleteplus/config/store_email');
        if ($storeMail) {
    ...
    public function webhook_service_call($observer)
    {
        try {
            ...
            if(function_exists('fsockopen')) {
                ...
                //Mage::log(print_r($res_obj, true), null, 'hycubecomplete.log', true);
                curl_close($client);
            }

        } catch (Exception $e) {
            Mage::log($e->getMessage(), null, 'hycubecomplete.log', true);
        }
    }
    ...
    protected function _getWebhookObjectUri($event_name, $observer)
    {
        ...
        return static::HYCUBECOMPLETEPLUS_WEBHOOK_URI.'?'.http_build_query($parameters, '', '&');
    }
