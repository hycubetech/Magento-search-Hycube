...
/**
 * Hycubecompleteplus_Autosuggest_Helper_Data
 *
 * NOTICE OF LICENSE
 *
 * This source file is subject to the Open Software License (OSL 3.0)
 ...
 * @link      http://opensource.org/licenses/osl-3.0.php
 */
class Hycubecompleteplus_Autosuggest_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    {
        return Mage::getModel('hycubecompleteplus_autosuggest/config');
    }
    ...
    public function getVersion()
    {
        return (string) Mage::getConfig()
            ->getModuleConfig('Hycubecompleteplus_Autosuggest')
            ->version;
    }
    ...
    public function getMultiStoreDataJson()
    {
        ...
        $ext = Mage::helper('hycubecompleteplus_autosuggest')->getVersion();
        ...
        $storeMail = $this->getConfigDataByFullPath(
            'hycubecompleteplus/config/store_email'
        );
    ...
    public function getExtensionConflict($all_conflicts = false)
    {
        ...
        $isp_module_name = 'Hycubecompleteplus_Autosuggest';
    ...
    private function setUpdateNeededForProduct(
        ...
        try {
            ...
            $is_table_exist = $write
                ->showTableStatus($table_prefix.'hycubecompleteplus_batches');
            ...
            $sql_fetch = 'SELECT * FROM '.
                $table_prefix.
                'hycubecompleteplus_batches WHERE product_id=? AND store_id=?';
            $updates = $read->fetchAll($sql_fetch, array($product_id, $store_id));

            if ($updates && (count($updates) != 0)) {
                $sql = 'UPDATE '.$table_prefix.
                    'hycubecompleteplus_batches  SET update_date=?,action=? WHERE product_id=? AND store_id=?';
                $write->query(
                    $sql,
                ...
                $sql = 'INSERT INTO '.
                    $table_prefix.
                    'hycubecompleteplus_batches (product_id,store_id,update_date,action,sku) VALUES (?,?,?,?,?)';
                $write->query(
                    $sql,
                    array($product_id,
                        $store_id,
    ...
    public function deleteProductFromTables($read, $write, $table_prefix, $product_id, $store_id)
    {
        $dt = strtotime('now');
        $sku = 'dummy_sku';
        $sqlFetch = 'SELECT * FROM '.$table_prefix.'hycubecompleteplus_batches WHERE product_id = ? AND store_id=?';
        $updates = $read->fetchAll($sqlFetch, array($product_id, $store_id));

        if ($updates && count($updates) != 0) {
            $sql = 'UPDATE '.$table_prefix.'hycubecompleteplus_batches SET update_date=?,action=? WHERE product_id = ? AND store_id = ?';
            $write->query($sql, array($dt, 'remove', $product_id, $store_id));
        } else {
            $sql = 'INSERT INTO '.$table_prefix.'hycubecompleteplus_batches  (product_id,store_id,update_date,action,sku) VALUES (?,?,?,?,?)';
    ...
    {
        Mage::log($log, null, 'hycubecompleteplus.log');
    }
    ...
    public function getServerEndPoint()
    {
        try {
            ...
            $tblExist = $write->showTableStatus(
                $_tableprefix.'hycubecompleteplus_config'
            );
            ...
            $sql = 'SELECT * FROM `'.
                $_tableprefix.'hycubecompleteplus_config` WHERE `id` =1';
    ...
    public function setServerEndPoint($end_point)
    {
        try {
            ...
            $sqlFetch = 'SELECT * FROM '.$_tableprefix.
                'hycubecompleteplus_config WHERE id = 1';
            $updates = $write->fetchAll($sqlFetch);

            if ($updates && count($updates) != 0) {
                $sql = 'UPDATE '.$_tableprefix.
                    'hycubecompleteplus_config SET server_type=? WHERE id = 1';
                $write->query($sql, array($end_point));
            } else {
                Mage::log('cant update server_type', null, 'hycubecompleteplus.log');
            }
        } catch (Exception $e) {
            Mage::log($e->getMessage(), null, 'hycubecompleteplus.log');
        }
    }
    ...
    public function getErrormessage()
    {
        ...
        $tblExist = $write->showTableStatus($_tableprefix.'hycubecompleteplus_config');
    ...
    {
        $collection = Mage::getModel('hycubecompleteplus_autosuggest/pusher')->getCollection();
    ...
    {
        $collection = Mage::getModel('hycubecompleteplus_autosuggest/pusher')
            ->getCollection()
            ->addFilter('sent', 0);
    ...
    public function getPushUrl($id = null)
    {
        ...
        if (strpos($url, 'index.php') !== false) {
            ...
            $url = $url.'hycubecompleteplus/products/pushbulk/pushid/'.$id;
        } else {
            $url = $url.'index.php/hycubecompleteplus/products/pushbulk/pushid/'.$id;
        }
